import { ProductFetchedEvent, ScrapeOkazjeEvent } from "@core/events";
import { ListingPayload } from "@core/types";
import * as cheerio from "cheerio";

import { Controller, Inject, Logger } from "@nestjs/common";
import { ConfigService } from "@nestjs/config";
import { ClientProxy, EventPattern } from "@nestjs/microservices";

interface OkazjeConfig {
  baseUrl: string;
  maxRetries: number;
}

@Controller()
export class ScrapeOkazjeHandler {
  private readonly logger = new Logger(ScrapeOkazjeHandler.name);
  private readonly config: OkazjeConfig;

  constructor(
    @Inject("PRODUCT_FETCHED_EVENT") private readonly eventBus: ClientProxy,
    private readonly configService: ConfigService,
  ) {
    this.config = {
      baseUrl:
        this.configService.get<string>("OKAZJE_BASE_URL") ??
        "https://www.okazje.info.pl",
      maxRetries: Number.parseInt(
        this.configService.get<string>("OKAZJE_MAX_RETRIES") ?? "3",
      ),
    };
  }

  @EventPattern(ScrapeOkazjeEvent.name)
  async handle(event: ScrapeOkazjeEvent): Promise<void> {
    this.logger.log(`Handling Okazje scrape for query: ${event.query}`);

    try {
      const listings = await this.scrapeOkazje(event.query);

      const productFetchedEvent = new ProductFetchedEvent(
        listings,
        event.chatId,
        "okazje",
        true,
        event.eventId,
        event.totalEvents,
      );

      this.logger.log(
        `Emitting ProductFetchedEvent with ${listings.length.toString()} listings for chat ${event.chatId}`,
      );

      this.eventBus.emit(ProductFetchedEvent.name, productFetchedEvent);
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      this.logger.error(`Error scraping Okazje products: ${errorMessage}`);

      const productFetchedEvent = new ProductFetchedEvent(
        [],
        event.chatId,
        "okazje",
        false,
        event.eventId,
        event.totalEvents,
      );

      this.eventBus.emit(ProductFetchedEvent.name, productFetchedEvent);
    }
  }

  private async scrapeOkazje(query: string): Promise<ListingPayload[]> {
    const url = `${this.config.baseUrl}/search/?q=${encodeURIComponent(query)}`;

    this.logger.log(`Scraping Okazje for query: "${query}"`);

    try {
      const response = await fetch(url);

      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status.toString()}`);
      }

      const html = await response.text();
      this.logger.log(
        `Successfully scraped ${html.length.toString()} characters`,
      );

      const listings = this.parseProducts(html);
      this.logger.log(
        `Fetched ${listings.length.toString()} Okazje listings for query="${query}"`,
      );

      return listings;
    } catch (error) {
      this.logger.error(
        `Failed to scrape: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }

  private parseProducts(html: string): ListingPayload[] {
    const $ = cheerio.load(html);
    const listings: ListingPayload[] = [];

    // Each product is in a div with class "pB--cr"
    $(".pB--cr").each((index, element) => {
      try {
        const $element = $(element);
        const $link = $element.find("a.pB__href");

        // Extract title
        const title = $element
          .find(".pB__i--name .pB__href--text")
          .text()
          .trim();

        // Extract price
        const priceText = $element
          .find(".pB__i--price .priceRegular")
          .text()
          .trim();
        const priceNumeric = this.extractNumericPrice(priceText);

        // Extract shop name
        const shop = $element.find(".pB__i--shop").text().trim();

        // Extract image URL
        const imageUrl = $element.find(".pB__image--src").attr("src") ?? null;

        // Extract product URL (decode from data-r-hash)
        const redirectHash = $link.attr("data-r-hash") ?? "";
        const productUrl = this.decodeProductUrl(redirectHash);

        // Extract description
        const description = $element.find(".pB__i--desc").text().trim();

        // Extract parameters
        const parameters: Record<string, string> = {};
        $element.find(".pB__i--params p").each((_, parameter) => {
          const text = $(parameter).text().trim();
          const colonIndex = text.indexOf(":");
          if (colonIndex > 0) {
            const key = text.slice(0, colonIndex).trim();

            const value = text.slice(colonIndex + 1).trim();
            parameters[key] = value;
          }
        });

        // Only add if we have essential data
        if (Boolean(title) && Boolean(shop)) {
          listings.push({
            image: imageUrl ?? null,
            title,
            description:
              description === "" ? `${title} - ${shop}` : description,
            link: productUrl,
            price: {
              value: priceNumeric,
              label: priceText,
              currency: "PLN",
              negotiable: false,
            },
            category: parameters["Grupa produktowa"] ?? null,
            provider: shop,
            isFavorited: false,
          });
        }
      } catch (error) {
        this.logger.warn(
          `Failed to parse product at index ${String(index)}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    });

    return listings;
  }

  private extractNumericPrice(priceText: string): number | null {
    // Remove all non-numeric characters except comma and dot
    // Polish format: "1 729,00 zł" -> 1729.00
    const cleaned = priceText
      .replaceAll(/\s/g, "") // Remove spaces
      .replace("zł", "") // Remove currency
      .replace(",", ".") // Replace comma with dot
      .replaceAll(/[^\d.]/g, ""); // Remove everything except digits and dot

    const numeric = Number.parseFloat(cleaned);
    return Number.isNaN(numeric) ? null : numeric;
  }

  private decodeProductUrl(redirectHash: string): string {
    if (!redirectHash) {
      return "";
    }

    try {
      // The redirect hash is in format: redirect/?r=BASE64_ENCODED_JSON
      // Extract the base64 part after "r="
      const base64Match = /r=([^&]+)/.exec(redirectHash);
      if (base64Match == null) {
        return "";
      }

      // URL decode first (e.g., %3D -> =)
      let base64String = decodeURIComponent(base64Match[1]);

      // Decode URL-safe base64 (replace - with +, _ with /)
      base64String = base64String.replaceAll("-", "+").replaceAll("_", "/");

      // Add padding if needed (Base64 requires length to be multiple of 4)
      while (base64String.length % 4 !== 0) {
        base64String += "=";
      }

      // Decode base64 to get JSON string
      const decodedString = Buffer.from(base64String, "base64").toString(
        "utf8",
      );

      // Parse JSON to extract the actual URL
      const data = JSON.parse(decodedString) as { url?: string };
      const url = data.url ?? "";

      // Clean up affiliate/tracking URLs
      return this.cleanAffiliateUrl(url);
    } catch (error) {
      this.logger.warn(
        `Failed to decode product URL from "${redirectHash}": ${error instanceof Error ? error.message : String(error)}`,
      );
      return "";
    }
  }

  private cleanAffiliateUrl(url: string): string {
    if (!url) {
      return "";
    }

    try {
      // Handle TradeDoubler affiliate links (e.g., Empik)
      // Format: https://pdt.tradedoubler.com/click?...url(ENCODED_URL)
      if (url.includes("tradedoubler.com")) {
        const urlMatch = /url\(([^)]+)\)/.exec(url);
        if (urlMatch != null) {
          return decodeURIComponent(urlMatch[1]);
        }
      }

      // Handle other common affiliate patterns
      // Format: ?redirect=ENCODED_URL or &url=ENCODED_URL
      const redirectMatch = /[?&](redirect|url|target)=([^&]+)/.exec(url);
      if (redirectMatch != null) {
        const potentialUrl = decodeURIComponent(redirectMatch[2]);
        // Check if it's a valid URL
        if (potentialUrl.startsWith("http")) {
          return potentialUrl;
        }
      }

      return url;
    } catch (error) {
      this.logger.warn(
        `Failed to clean affiliate URL: ${error instanceof Error ? error.message : String(error)}`,
      );
      return url;
    }
  }
}
